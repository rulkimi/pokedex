...
export const useGenerations = () => {
  ...
  const getLimitAndOffsets = (generation: number): Generation => {
    const generationLimits: { [key: number]: Generation } = {
    ...
    if (!generationLimits[generation]) {
      throw new Error(`Gen ${generation} doesn't exist in the Pokemon world!`);
    }

    return generationLimits[generation];
  }

  return { getLimitAndOffsets }
}